...
var MainCalendar = /** @class */ (function (_super) {
    ...
    function MainCalendar(props) {
        ...
        _this_1.connectorCollection = new ConnectorCollection();
        return _this_1;
    }
    MainCalendar.prototype.init = function () {
        ...
        _this.calendar = new Calendar(_this.element, {
            ...
            select: function (event) {
                var start = DateTime.getSystemDate(event.start);
                var end = DateTime.getSystemDate(event.end);
                if (event.allDay) {
                    start.setHours(12);
                    end.setHours(12);
                }
                    ...
                    var calendarModel = kendo.observable({
                        ...
                        changeCalendar: function (e) {
                            ...
                            if (e.data.calendar_id) {
                                var loader_1 = new Loading('create-event-dialog');
    ...
    MainCalendar.prototype.createEvent = function (event, connector, form) {
        ...
        RedooAjax(App.SCOPE_NAME).postAction('EventAdd', event.getAll(), false, 'json').then(function (response) {
    ...
    MainCalendar.prototype.eventChange = function (event) {
        ...
        var start = DateTime.getSystemDate(event.event.start);
        var end;
        if (event.event.end) {
    ...
    MainCalendar.prototype.updateEvent = function (event, form, connection, calendar) {
        ...
        this.loading.startLoading();
        var _this = this;
        RedooAjax(App.SCOPE_NAME).postAction('EventUpdate', event.getAll(), false, 'json').then(function (response) {
            if (response.status) {
                event.setData(response.event);
...
export { MainCalendar };
//# sourceMappingURL=MainCalendar.js.map