...
class VTEntity
{
     ...
     * If Module contains Inventory, you get \RedooCalendar\VTInventoryEntity
     * @static
     ...
     * @example
     * <br/>
     * $record = \RedooCalendar\VTEntity::getForId(815);<br/>
     * $record = \RedooCalendar\VTEntity::getForId(815, 'Accounts');
     */
...
//            $ownerFields = $metaHandler->getOwnerFields();
    ...
    public function save()
    {
        ...
        //update is not working properly, so there is an optional solution(to be fixed)
        $this->updateData();
...
//            $focus->column_fields = \DataTransform::sanitizeCurrencyFieldsForInsert($focus->column_fields, $metaHandler);
            ...
            foreach ($focus->column_fields as $key => $value) {
                ...
                //there was some bugs with German sybmols on update, and this fixes it somehow
                if($key == "label")
                {
                    continue;
                }
...
//            $this->_environment = $tmpEnvironment;
    ...
    /*
        Function works with database directly using data from $this->_data, which has actual data.
        This solution isn`t good, and needs to be replaced with something better
    */
    public function updateData()
    {
        //columns for each table we need to update
        $tables = [ 
            "vtiger_activity" =>[
                 "date_start" => true,
                 "due_date" => true,
        ...
        //identificators for table we need to update
        $table_ids = [
        ...
        //each table from array needs to be updated
        foreach($tables as $table => $values)
        {
            //forming sql request to change needed table
            $sql = "UPDATE {$table} SET {$table_ids[$table]} = {$table_ids[$table]}";

            foreach($values as $column => $needs_quotes)
            {   
                //if column is empty, there will be sql-error
                if($this->_data[$column] != null)
                {
                    $sql .= ", {$column}=";

                    //if column type is varchar or date, it needs quotes. but if int or float, it dont need it
                    if($needs_quotes)
                    {
                        $sql .= "'{$this->_data[$column]}'";
                    }
                    else
                    {
                        $sql .= "{$this->_data[$column]}";
                ...
                //var_dump($needs_quotes);
            }

            $sql .= " WHERE {$table_ids[$table]} = {$this->_id}";
            $db->pquery($sql);
     ...
     * Create new Record
     *
     * @param $module string ModuleName of new Recor
    ...
    public function redirectToCreationForm()
    {
        ...
        ?>
        <form method="POST" id="submitCreationForm" name="submitCreationForm"
              action="index.php?module=<?php echo $this->getModuleName(); ?>&view=Edit">
