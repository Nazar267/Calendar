...
class VtUtils
{
    ...
    {
        /*       if(!class_exists('RedooDashboard_Crypt_Blowfish')) {
       ...
       */
        ...
        $constant = strtoupper(basename(dirname(__FILE__))) . '_ROOTPATH';
        if (empty($key)) {
            $key = file_get_contents(constant($constant) . DIRECTORY_SEPARATOR . 'cryptkey.dat');

            if (!file_exists(constant($constant) . DIRECTORY_SEPARATOR . 'cryptkey.dat')) {
...
//        $bf->paddable = false;
        $bf->setKey($key);
        $rawData = $bf->decrypt(base64_decode($value));
  ...
  */
        ...
        if (empty($key)) {
            ...
            if (!file_exists(constant($constant) . DIRECTORY_SEPARATOR . 'cryptkey.dat')) {
                file_put_contents(constant($constant) . DIRECTORY_SEPARATOR . 'cryptkey.dat', self::generate_password(50));
            }
            $key = file_get_contents(constant($constant) . DIRECTORY_SEPARATOR . 'cryptkey.dat');
            $traceData = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2);
            $key .= substr(md5(substr(md5(str_replace(vglobal('root_directory'), '', $traceData[0]['file'])), 0, 10)), 0, 5);
    ...
    public static function getTextColor($backgroundColor)
    {
        if (strpos($backgroundColor, '#') !== false) {
            ...
            $backgroundColor = sscanf($backgroundColor, "rgb(%d, %d, %d)");
            $rgb = array(
...
//7f18c166060f17d0ce582a4359ad1cbc
    ...
    {
        return vglobal('root_directory') . 'modules' . DS . 'RedooCalendar' . DS . 'extends' . DS . 'additional' . DS . $additionalKey . DS;
    }
}
